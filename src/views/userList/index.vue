<template>
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        >
            ...
            <el-col :span="8">
              <div style="text-align: center">
                <div
                  ...
                  :style="{
                    color:
                      recordForm.totalScore >= 10
                        ? '#F56C6C'
                        : recordForm.totalScore >= 5
                        ? '#E6A23C'
                        : '#67C23A',
                  }"
                >
                  {{ recordForm.totalScore }}
                </div>
          ...
          <div style="margin-top: 15px; text-align: center">
            <el-tag
              :type="
                recordForm.riskLevel === '高危'
                  ? 'danger'
                  : recordForm.riskLevel === '中危'
                  ? 'warning'
                  : 'success'
              "
              size="medium"
          ...
          <el-col :span="24">
            <el-form-item label="病历图片" prop="imgUrl">
              <el-upload
                ...
                :on-remove="handleRecordImgRemove"
                :before-upload="beforeMedicalImgUpload"
                :limit="5"
                multiple
                :data="{ bizType: 'record' }"
                :disabled="!isRecordEdit && recordTitle === '查看就诊记录'"
        ...
        <el-form-item label="诊断结果" prop="diagnosisResult">
          <el-input
            v-model="recordForm.diagnosisResult"
            :rows="4"
            type="textarea"
            placeholder="请输入诊断结果"
            :disabled="!isRecordEdit && recordTitle === '查看就诊记录'"
        ...
        <el-form-item label="治疗建议" prop="treatmentSuggestion">
          <el-input
            v-model="recordForm.treatmentSuggestion"
            :rows="4"
            type="textarea"
            placeholder="请输入治疗建议"
            :disabled="!isRecordEdit && recordTitle === '查看就诊记录'"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          v-if="isRecordEdit || recordTitle === '新增就诊记录'"
          type="primary"
  ...
  methods: {
    ...
    handleViewRecord(row) {
      ...
      if (row.imgUrl) {
        ...
        if (cleanUrlStr) {
          this.recordImgList = cleanUrlStr
            .split(",")
            .map((url) => {
    ...
    submitRecordForm() {
      ...
      this.$refs["recordForm"].validate((valid) => {
        if (valid) {
          const url = this.isRecordEdit
            ? "/system/score/record"
            : "/system/score/record/submit";
    ...
    /** 获取医生列表 */
    getDoctorList() {
      listDoctor({ pageNum: 1, pageSize: 1000 })
        .then((response) => {
          if (response.code === 200) {
    ...
    handleView(row) {
      ...
      getPatient(patientId).then((response) => {
        ...
        if (this.viewForm.imgUrl) {
          ...
          if (cleanUrlStr) {
            this.medicalRecordImgList = cleanUrlStr
              .split(",")
              .map((url) => {
                const cleanUrl = url.trim();
    ...
    handleAdd() {
      ...
      if (currentUser && currentUser.userId) {
        this.form.doctorId = Number(currentUser.userId);
        const doctor = this.doctorList.find(
          (item) => item.value == currentUser.userId
        );
        if (doctor) {
    ...
    handleUpdate(row) {
      ...
      getPatient(patientId).then((response) => {
        this.form = {
          ...response.data,
          doctorId: Number(response.data.userId),
        };
        // 确保 doctorId 是数字类型
        ...
        // 优先使用 userId 回显主治医师
        // if (this.form.userId) {
        //   this.form.doctorId = this.form.userId;
        // }
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.patientId != undefined) {
            console.log(this.form, "this,form=====更新");
